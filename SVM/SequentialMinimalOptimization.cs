...
namespace LogisticRegession
{
  public class SequentialMinimalOptimization
  {
    ...
    public double Run(bool computeError)
    {
      ...
      machine.Weights = new double[inputs[0].Length];
      machine.SupportVectorsTwoSides = new double[inputs[0].Length][];
      var isPositiveAssigned = false;
      var isNegativeAssigned = false;
      for (int i = 0; i < vectors; i++)
      {
        int j = indices[i];
        machine.SupportVectors[i] = inputs[j];
        var a = inputs[j].Multiply(alpha[j]*outputs[j]);
        machine.Weights = machine.Weights.Add(a);
        if (outputs[j] > 0 && !isPositiveAssigned)
        {
          machine.SupportVectorsTwoSides[0] = inputs[j];
          isPositiveAssigned = true;
        }
        if (outputs[j] < 0 && !isNegativeAssigned)
        {
          machine.SupportVectorsTwoSides[1] = inputs[j];
          isNegativeAssigned = true;
        }
        //machine.Weights[i] = alpha[j]*outputs[j];
        //machine.Alphas[i] = alpha[j];
      }
